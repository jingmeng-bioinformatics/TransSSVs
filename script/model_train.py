...
if __name__ == "__main__":
    ...
    vaild_data=np.load(args.vaild_dir+'/data.'+filename+'.npy')
    vaild_labels=np.load(args.vaild_dir+'/label.'+filename+'.npy')
    ...
    if args.indicator=='fine_tune':
        ...
        pass
    checkpoiner=tf.keras.callbacks.ModelCheckpoint(filepath=args.save_dir+'/model_{epoch:02d}',monitor='val_loss',save_weights_only=True,verbose=1)
    history_callback=TransSSVs_model.fit(
        x=data_only, y=labels,
        batch_size=args.batch_size, 
        epochs=args.epoch, 
        verbose=1, 
        callbacks=[checkpoiner],
        shuffle=True,
        validation_data=(vaild_data,vaild_labels))
    print(TransSSVs_model.predict(data_only))
    pandas.DataFrame(history_callback.history).to_csv(args.save_dir + '/log.csv')
