...
parser.add_argument('--Candidate_somatic_sites', required=True, metavar='file', help='a file of candidate or validated somatic sites')
...
def main(args):
    with xopen(args.Tumor_Normal_mpileup, 'rt') as TN, open(args.Candidate_somatic_sites, 'rt') as Cs:
        enume_Cs = enumerate(Cs) 
        enume_TN = enumerate(TN)
        if os.path.exists(args.path+'/'+args.filename_1+'_npyarray/'):
            pass
        else:
