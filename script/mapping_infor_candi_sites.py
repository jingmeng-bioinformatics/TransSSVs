    ...
    with xopen(args.Tumor_Normal_mpileup, 'rt') as TN, open(args.Candidate_validated_somatic_sites, 'rt') as Cs:
        ...
        for line in enume_Cs:
            ...
            if args.indicator == 'training':
                ...
                pass
            i+=1
            #there are 1000 sites in 1 npy file
            if i <= 100:
                if args.indicator == 'training':
                    label_array=np.append(label_array,label_npy)
                else:
                    pass
                ...
                if args.indicator == 'training':
                    np.save(args.path+'/'+args.filename_1+'_npyarray/'+'label.'+args.filename_1+'_'+str(j)+'.npy',label_array)
                    label_array=np.zeros((0),dtype='int64')
                    label_array=np.append(label_array,label_npy)
                else:
                    pass
                np.save(args.path+'/'+args.filename_1+'_npyarray/'+'data.'+args.filename_1+'_'+str(j)+'.npy',npyarray_1)
                npyarray_1=np.zeros((shape_y,52,0),dtype='float32')
