...
file_mutect2_snp <- read.table(paste(file_path,name,'_snp_3.vcf',sep=''),sep='\t')

temp=unlist(strsplit(file_mutect2_snp[, 8], ";"))
temp=temp[grep("SSC", temp)]
temp <- gsub("SSC=","", temp)
...
write.table(snp_data,paste('figure/roc/data/data/varscan2_',name,'_snp.txt',sep=''),row.names=F,col.names=F,sep='\t',quote=F)

file_mutect2_indel <- read.table(paste(file_path,name,'_indel_3.vcf',sep=''),sep='\t')

temp=unlist(strsplit(file_mutect2_indel[, 8], ";"))
temp=temp[grep("SSC", temp)]
temp <- gsub("SSC=","", temp)

#file_mutect2_indel[,12]=gsub(".*SomaticEVS=","", file_mutect2_snp[, 8])
...
write.table(indel_data,paste('figure/roc/data/data/varscan2_',name,'_indel.txt',sep=''),row.names=F,col.names=F,sep='\t',quote=F)
