...
file_mutect2_snp <- read.table(paste(file_path,name,'_snp_4.vcf',sep=''),sep='\t')

temp=unlist(strsplit(file_mutect2_snp[, 8], ";"))
temp=temp[grep("SomaticEVS", temp)]
temp <- gsub("SomaticEVS=","", temp)


file_mutect2_snp[,12] <-temp
#file_mutect2_snp <- select(file_mutect2_snp,V1,V2,V12)

#file_mutect2_snp[,12]=gsub(".*SomaticEVS=","", file_mutect2_snp[, 8])
...
write.table(snp_data,paste('figure/roc/data/data/strelka2_',name,'_snp.txt',sep=''),row.names=F,col.names=F,sep='\t',quote=F)

file_mutect2_indel <- read.table(paste(file_path,name,'_indel_0.vcf',sep=''),sep='\t')

temp=unlist(strsplit(file_mutect2_indel[, 8], ";"))
temp=temp[grep("SCORE", temp)]
temp <- gsub("SCORE=","", temp)


file_mutect2_indel[,12] <-temp
#file_mutect2_indel <- select(file_mutect2_indel,V1,V2,V12)


#file_mutect2_indel[,12]=gsub(".*SCORE=","", file_mutect2_indel[, 8])
...
write.table(indel_data,paste('figure/roc/data/data/varnet_',name,'_indel.txt',sep=''),row.names=F,col.names=F,sep='\t',quote=F)
