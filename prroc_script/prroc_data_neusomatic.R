...
file_mutect2_snp <- read.table(paste(file_path,name,'_snp_1.vcf',sep=''),sep='\t')

#file_mutect2_snp[,12]=gsub(".*SomaticEVS=","", file_mutect2_snp[, 8])

file_mutect2_snp <- select(file_mutect2_snp,V1,V2,V6)
...
write.table(snp_data,paste('figure/roc/data/data/neusomatic_',name,'_snp.txt',sep=''),row.names=F,col.names=F,sep='\t',quote=F)

file_mutect2_indel <- read.table(paste(file_path,name,'_indel_1.vcf',sep=''),sep='\t')

#file_mutect2_indel[,12]=gsub(".*TLOD=","", file_mutect2_indel[, 8])

file_mutect2_indel <- select(file_mutect2_indel,V1,V2,V6)
...
write.table(indel_data,paste('figure/roc/data/data/neusomatic_',name,'_indel.txt',sep=''),row.names=F,col.names=F,sep='\t',quote=F)
